...
| 機能          | 内容                                |
| ----------- | --------------------------------- |
| ユーザー登録      | Deviseでの認証機能（メール登録・ログイン）          |
| Googleログイン  | OmniAuth（Google OAuth 2.0）による外部認証 |
| お店一覧表示      | Google Maps APIを利用した地図表示          |
| オートコンプリート機能 | Google Places APIで住所や店名を自動補完      |
| お気に入り機能     | 気になるお店を保存・解除できる                   |
| カテゴリ・シーン検索  | Ransackを使った複合検索                   |
| 画像投稿        | Active Storageで店舗画像を登録            |
| レスポンシブ対応    | Tailwindでスマホ最適化                   |
